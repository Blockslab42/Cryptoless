...
describe('nft contract', function () {
    ...
    describe('Public Sale', async function () {
        ...
        it('Should withdraw the money', async function () {
            ...
            await expect(
                await contract.connect(addr3).mintNFT(9, 111, {
                    value: 1,
                })
            );
            await expect(
                await contract.connect(addrs[4]).mintNFT(9, 111, {
                    value: 1,
                })
            );
            await expect(
                await contract.connect(addrs[5]).mintNFT(9, 111, {
                    value: 1,
                })
            );
            await expect(
            ...
            await expect(
                await contract.connect(addrs[7]).mintNFT(9, 111, {
                    value: 1,
                })
            );
            await expect(
                await contract.connect(addrs[8]).mintNFT(9, 111, {
                    value: 1,
                })
            );
            await expect(
                await contract.connect(addrs[9]).mintNFT(9, 111, {
                    value: 1,
                })
            );
